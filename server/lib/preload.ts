...
const CONCURRENCY = Math.max(1, Number(process.env.PRELOAD_CONCURRENCY || 2));
...
async function loadFilterCombinations(): Promise<Record<string, unknown>[]> {
  ...
  const years = options.years.length ? options.years.map(String) : ['2026'];
...
export function startPreloadScheduler(): void {
  ...
  status.enabled = true;
  console.log(`[preload] scheduler started — every ${INTERVAL_MS / 60000} min`);
  void runPreloadCycle();
}
