...
def _sanitise(d):
    ...
    def ins(i):
        ...
        t=_ss(i.get("text") or i.get("insight",""),400)
    ...
    def alrt(a):
        ...
        # site field may come back as site/Site/SITE/site_name/location
        site = (a.get("site") or a.get("Site") or a.get("SITE") or
                a.get("site_name") or a.get("location") or "")
        site = _ss(str(site), 60)
        if not site or site in {"null","None","[site name]","[site name from data]"}:
            return None
        sv = a.get("severity","Medium")
        return {
            "kpi":           _ss(a.get("kpi",""),60),
            "site":          site,
            "current_value": _ss(a.get("current_value") or a.get("site_value") or "",25),
            "national_avg":  _ss(a.get("national_avg") or a.get("avg") or "",25),
            "gap":           _ss(a.get("gap") or a.get("gap_pp") or "",40),
            "severity":      sv if sv in {"Critical","High","Medium","Low"} else "Medium",
            "ai_insight":    _ss(a.get("ai_insight") or a.get("insight") or "",600),
...
QUESTIONS = {
  ...
  "dt_kpi": (
    ...
    '"mttr_hrs":"X.Xh",'
    '"mtbf_hrs":"X.Xh",'
    '"insight":"WHY: Unplanned DT is X.X% — X.Xpp ABOVE/BELOW 5% target. Primary driver is [top RSN] on [top line] accounting for Xh this period. WHEN: problem is most severe on [Shift/Day] and has been [rising/stable] for X weeks. HOW TO FIX: address [RSN] on [line] before [Shift] — this recovers X.Xpp and brings total DT to X.X% vs 5% target within [timeframe]."}'
  ),
  ...
  "dt_reasons": (
    ...
    '{"line":"[line name]","pct":0,"color":"gray"},'
    '{"line":"Others","pct":0,"color":"gray"}],'
    '"insight":"WHY: [exact RSN] is the #1 cause at Xh ([X]% of all unplanned DT), concentrated on [line]. Root cause is [equipment/process issue inferred from RSN name]. WHEN: occurs most on [Shift] and has [increased/been consistent] over recent periods. HOW TO FIX: schedule targeted PM for [RSN] on [line] before next [Shift] — resolving this single issue reduces total DT from X.X% to X.X%, saving Xh/week."}'
  ),
  ...
  "dt_comments": (
    ...
    '"insight":"[DIAGNOSTIC: operators flagged [exact theme] X times — verbatim: [phrase from Comment_Text]. Points to [equipment/process] failing repeatedly. WHEN: X% of these comments come from [Shift/Day], confirming a recurring not random pattern. HOW TO FIX: [specific corrective action — e.g. lubrication schedule, part replacement] before next [Shift/Day]. Expected: reduce [theme] incidents by ~X% next week."}'
  ),
  ...
  "dt_shift": (
    ...
    '"insight":"[DIAGNOSTIC: Last [Shift] DT was X.X% — [WON/LOST] vs 5% target. WHY: [Worst Shift] averages Xh vs [Best Shift] Xh — Xpp gap caused by [handover gaps/equipment state/operator experience]. WHEN: gap has persisted for X periods, worst on [specific day if pattern]. HOW TO FIX: (1) pair senior operator with [Worst Shift] for 2 weeks, (2) add 10-min pre-shift equipment check. Closing gap saves Xh/week and recovers X.Xpp DT."}'
  ),
  ...
  "dt_trend": (
    ...
    '"severity":"anomaly|critical|warn"},'
    '"insight":"WHY: DT [RISING/FALLING] W1→W5 from X.Xh to X.Xh — acceleration at W[X] when [RSN spiked]. WHEN: W6 projected at X.Xh — [WILL/WILL NOT] breach 5% target. HOW TO FIX: [if rising] escalate [top RSN] to plant manager today, schedule emergency PM before W6; [if falling] sustain actions and confirm recovery at W6."}'
  ),
  ...
  "waste_kpi": (
    ...
    '"insight":"WHY: Waste is X.X% — X.Xpp [ABOVE/BELOW] 5% target. Driven by [WST_TYP] at X% of waste lbs from [SECRY_AREA]. Root process issue is [infer from WST_TYP — e.g. rework rate, yield loss on specific equipment]. WHEN: trend is [rising/falling], W7 projected at X.X%. Problem is worst on [Shift/Line]. HOW TO FIX: reduce [WST_TYP] in [SECRY_AREA] by [specific action] — a X% reduction recovers X.Xpp and saves ~$X,XXX this week."}'
  ),
  ...
  "waste_trend": (
    ...
    '"severity":"anomaly|critical|warn"},'
    '"insight":"WHY: Waste % [RISING/FALLING] W1→W6 from X.X% to X.X% — inflection at W[X] driven by [WST_TYP spike/process change from data]. WHEN: W7 projected at X.X% — [WILL/WILL NOT] breach 5% target. HOW TO FIX: [if rising] inspect [specific equipment in SECRY_AREA] and tighten [WST_TYP] handling before W7 to avoid breach; [if falling] sustain current controls through W7."}'
  ),
  ...
  "opportunities": (
    ...
    '"insight":"[PRESCRIPTIVE: Top priority — [specific action naming RSN/Line] saves Xh DT this week. PREDICTIVE: without action, cumulative monthly loss = Xh / $X,XXX. Assign to [role] today — measurable impact in 48h. WHY top sites underperform: [top site] is Xpp above national avg because [RSN/process from data]. HOW TO FIX: (1) [specific action for top site] recovers Xh/$X,XXX this week. (2) [action for 2nd site]."}'
  ),
  ...
  "alerts_dt": (
    ...
    '"site":"[exact site name from data]",'
    '"current_value":"X.X%",'
    '"national_avg":"X.X%",'
    '"gap":"+X.Xpp above national avg",'
    '"severity":"Critical|High|Medium",'
    '"ai_insight":"WHY: [site] DT is X.X% because [top RSN] accounts for Xh — [specific equipment/process]. WHEN: pattern strongest on [Shift/Day], recurring since [timeframe from data]. HOW TO FIX: assign [specific team] to [specific RSN on specific line] before next shift — this alone recovers X.Xpp and saves ~Xh/week.",'
    '"contributors":[{"name":"[RSN from data]","value":"Xh","pct":0},{"name":"[2nd RSN]","value":"Xh","pct":0}]},'
    '{"kpi":"Total Unplanned Downtime %","site":"[2nd site]","current_value":"X.X%","national_avg":"X.X%","gap":"+X.Xpp","severity":"High","ai_insight":"WHY: [cause]. WHEN: [pattern]. HOW TO FIX: [specific action].","contributors":[]},'
    '{"kpi":"Total Unplanned Downtime %","site":"[3rd site]","current_value":"X.X%","national_avg":"X.X%","gap":"+X.Xpp","severity":"Medium","ai_insight":"WHY: [cause]. WHEN: [pattern]. HOW TO FIX: [specific action].","contributors":[]}]}'
  ),
  ...
  "alerts_waste": (
    ...
    '"gap":"+X.Xpp above national avg",'
    '"severity":"Critical|High|Medium",'
    '"ai_insight":"WHY: [site] waste is X.X% because [WST_TYP] in [SECRY_AREA/DEPT] accounts for X% of total waste lbs — [root process issue]. WHEN: highest on [Shift/Day/Period], trend [rising/stable] over last X weeks. HOW TO FIX: target [WST_TYP] reduction in [specific area] — a X% reduction recovers X.Xpp and saves ~$X,XXX/week.",'
    '"contributors":[{"name":"[WST_TYP from data]","value":"X,XXXlbs","pct":0},{"name":"[2nd WST_TYP]","value":"X,XXXlbs","pct":0}]}]}'
  ),
...
def get_dashboard_parallel(filters):
    ...
    QUESTION_TIMEOUT = int(os.getenv("SUPERVISOR_QUESTION_TIMEOUT","600"))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        try:
            ...
            # Overall timeout — collect whatever finished, skip the rest
            print(f"[supervisor]   ⚠ Overall timeout {QUESTION_TIMEOUT}s — collecting partial results")
            for future, key in futures.items():
                if key not in results:
                    if future.done():
                        ...
                        print(f"[supervisor]   {key} did not finish — skipping")
                        results[key] = {}
