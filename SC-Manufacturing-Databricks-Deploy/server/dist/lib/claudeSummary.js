import { buildFilterContext, buildMetricsGroundingContext } from './summaryPrompts.js';
...
export function claudeEndpoint() {
    return (process.env.CLAUDE_SERVING_ENDPOINT
...
function claudeApiMode() {
    const mode = String(process.env.CLAUDE_API_MODE || 'auto').toLowerCase();
    if (mode === 'anthropic' || mode === 'messages')
        return 'anthropic';
    if (mode === 'invocations' || mode === 'chat')
        return 'invocations';
    return 'auto';
}
export function isRoboticTemplateSummary(text) {
...
function topEntries(obj, n = 5) {
    ...
    if (typeof entries[0]?.[1] === 'number') {
        return Object.fromEntries(entries.sort((a, b) => Number(b[1]) - Number(a[1])).slice(0, n));
    }
    return Object.fromEntries(entries.slice(0, n));
}
export function compactMetrics(metrics, entityType) {
    const base = {
        kpis: metrics.kpis,
    ...
    if (!entityType || entityType === 'dow') {
        base.dow_sample = Object.fromEntries(Object.entries(metrics.dow_by_day_week || {}).slice(0, 3));
    }
    return base;
}
    ...
    if (predictions?.[0]) {
        ...
        if (p && typeof p === 'object') {
            ...
            if (typeof po.content === 'string')
                return po.content.trim();
            if (Array.isArray(po.candidates)) {
                const c = po.candidates[0];
...
function parseBatchNarratives(raw) {
    const text = raw.replace(/```json\s*|```\s*/g, '').trim();
    try {
        const d = JSON.parse(text);
        const out = {};
        for (const tab of ['overview', 'category', 'line', 'dow', 'reason']) {
            const v = d[tab];
            if (typeof v === 'string' && v.trim())
                out[tab] = v.trim();
            else if (v && typeof v === 'object' && typeof v.narrative === 'string') {
                out[tab] = String(v.narrative).trim();
            }
        }
...
async function invokeClaudeInvocations(messages) {
    const h = host();
    ...
    if (!text)
        throw new Error('Claude invocations returned empty content');
    console.log(`[claude] ✓ invocations ${text.length} chars in ${Date.now() - started}ms`);
    return text;
}
async function invokeClaudeAnthropic(messages) {
    const h = host();
    ...
    if (!text)
        throw new Error('Claude anthropic returned empty content');
    console.log(`[claude] ✓ anthropic ${text.length} chars in ${Date.now() - started}ms`);
    return text;
}
export async function invokeClaude(messages) {
    const mode = claudeApiMode();
    if (mode === 'anthropic')
        return invokeClaudeAnthropic(messages);
    if (mode === 'invocations')
        return invokeClaudeInvocations(messages);
    try {
        return await invokeClaudeInvocations(messages);
    }
    catch (first) {
        console.warn(`[claude] invocations failed (${first.message.slice(0, 120)}) — trying anthropic/messages`);
        return invokeClaudeAnthropic(messages);
    }
}
export async function getClaudeTabSummary(entityType, filters, metrics) {
...
export async function getClaudeBatchSummaries(filters, metrics) {
    const filterCtx = buildFilterContext(filters);
