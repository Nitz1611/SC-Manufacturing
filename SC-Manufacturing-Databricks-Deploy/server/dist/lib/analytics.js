...
import { applySiteFilter, buildMetricsFromSql, metricsFromCache, metricsFromDashboardCache, queryResultForKey, } from './metricsTransform.js';
const MEMORY_TTL_MS = Number(process.env.METRICS_MEMORY_CACHE_MINUTES || 15) * 60 * 1000;
const PERIOD_VARIANTS = ['week', 'month', 'quarter', 'fiscal_year'];
const memoryCache = new Map();
let lastSqlError = null;
let lastSqlSuccessAt = null;
export function databricksConfigured() {
    return sqlConfigured();
}
export function getLastSqlError() {
    return lastSqlError;
}
export function getLastSqlSuccessAt() {
    return lastSqlSuccessAt;
}
function rememberSqlError(err) {
    const msg = err.message || String(err);
    lastSqlError = msg;
    throw new Error(`Live SQL failed for ${resolveMetricView()}: ${msg}. ` +
...
async function loadMetricsFromSql(norm, onProgress) {
    onProgress?.('Querying KPIs, sites, and trends…');
    const [kpis, siteKpis, periodTrend, siteByPeriod, reasons, dow, topLines, shiftComparison, filterOptions,] = await Promise.all([
...
function loadPriorSqlCache(filters) {
...
function loadMetricsFromDemoFallback(filters) {
    const norm = normalizeParams(filters);
    const key = coarseCacheKey(norm);
    const cached = cacheGet(key);
    if (cached?.kpis && cached.meta?.source === 'sql') {
    ...
    if (dashboard) {
        return applySiteFilter(metricsFromDashboardCache(dashboard, norm), norm.site);
    }
    return applySiteFilter(metricsFromCache(norm), norm.site);
}
function getMemoryCached(norm) {
    const key = coarseCacheKey(norm);
    const hit = memoryCache.get(key);
    if (!hit || Date.now() - hit.ts > MEMORY_TTL_MS)
        return null;
    return applySiteFilter(hit.data, norm.site);
}
function setMemoryCached(norm, metrics) {
...
export function storeMetricsBundleToCache(norm, metrics) {
    for (const period of PERIOD_VARIANTS) {
        const keyNorm = { ...norm, period };
        const scoped = applySiteFilter(metrics, keyNorm.site);
        setMemoryCached(keyNorm, scoped);
        cacheSet(coarseCacheKey(keyNorm), scoped);
    }
}
...
export function warmMemoryCacheFromDisk() {
    let loaded = 0;
    for (const entry of cacheLoadAllMetrics()) {
        try {
            const parsed = JSON.parse(entry.key.replace(/^metrics_/, ''));
            const norm = {
            ...
            if (!memoryCache.has(key)) {
                memoryCache.set(key, { data: entry.data, ts: entry.ts });
                loaded++;
            }
        }
        catch {
...
export async function getMetricsBundle(filters = {}) {
    const norm = normalizeParams(filters);
    ...
    if (sqlConfigured()) {
        try {
            ...
            console.error('[analytics] Live SQL failed:', err.message);
    ...
    if (!consoleDemoMode()) {
        throw new Error('Live SQL is not configured. Set DATABRICKS_* in .env, or set CONSOLE_DEMO_MODE=true to enable demo data.');
    }
    const demo = loadMetricsFromDemoFallback(filters);
    setMemoryCached(norm, demo);
    return demo;
}
export async function runAnalyticsQuery(queryKey, params) {
    ...
    if (sqlConfigured()) {
        try {
            ...
            if (prior) {
                ...
                return { rows: result.rows || [], source: 'cache', cached: true };
            }
            throw e;
        }
    }
...
export function metricsBundleToConsolePayload(metrics, extras = {}) {
...
export async function verifyMetricViewAccess() {
    if (!sqlConfigured()) {
