...
export function envSearchPaths() {
    ...
    if (process.env.DOTENV_PATH)
        add(process.env.DOTENV_PATH);
    for (const p of walkEnvPaths(repoRoot()))
        add(p);
    for (const p of walkEnvPaths(process.cwd()))
        add(p);
    return ordered;
}
...
export function sqlEnvStatus() {
    ...
    const missing = [];
    if (!host.trim())
        missing.push('DATABRICKS_HOST or DATABRICKS_SERVER_HOSTNAME');
    if (!token.trim())
        missing.push('DATABRICKS_PAT_TOKEN');
    if (!warehouse.trim())
