...
import { buildSupervisorPrompt } from './summaryPrompts.js';
...
export function supervisorConfigured() {
    return Boolean(host() && token() && endpoint());
}
function supervisorDatabricksOptions() {
    const opts = {};
    if (LONG_TASK)
        opts.long_task = true;
    return opts;
}
function isTimeoutResponse(text) {
    const lower = text.toLowerCase();
    return ((lower.includes('time out') || lower.includes('time limit') || lower.includes('timed out')) &&
        (lower.includes('continue') || lower.includes('would you like') || lower.includes('shall i')));
}
function findTaskContinueCheckpoint(raw) {
...
function parseJsonBlock(raw) {
    ...
    if (best)
        return unwrapResponse(best);
    if (looksLikeSseStream(cleaned))
        return {};
    return { narrative: cleaned.slice(0, 500) };
}
function unwrapResponse(d) {
    if (Array.isArray(d.output)) {
        ...
        if (DEBUG)
            console.log(`[supervisor] found ${allTexts.length} text block(s) in output`);
        if (allTexts.length) {
    ...
    if (typeof d.output === 'string')
        return parseJsonBlock(d.output);
    if ('narrative' in d || 'key_insights' in d || 'observations' in d || 'insight' in d)
        return d;
    return d;
}
...
function looksLikeSseStream(raw) {
    return raw.includes('data:') && (raw.includes('response.output_text.delta') ||
...
function parseSupervisorSseStream(raw) {
...
async function readResponseBody(resp) {
    ...
    while (true) {
        ...
        chunks.push(chunk);
        if (DEBUG)
            console.log(`[supervisor] chunk: ${chunk.slice(0, 150)}`);
    }
    return chunks.join('');
}
async function postOnce(input) {
    ...
    console.log(`[supervisor] -> ${endpoint()} | stream=true long_task=${Boolean(databricksOptions.long_task)}`);
    const body = {
        input,
        stream: true,
    };
...
async function callSupervisor(messages, metrics) {
    ...
    for (let attempt = 0; attempt < MAX_CONTINUATIONS; attempt++) {
        ...
        raw = assembled;
        if (DEBUG)
            console.log(`[supervisor] response ${raw.length} chars: ${raw.slice(0, 300)}`);
...
function isGarbageNarrative(text) {
    const t = text.trim();
    return (looksLikeSseStream(t) ||
        t.startsWith('data:') ||
...
function extractNarrative(raw) {
    ...
    if (typeof parsed.narrative === 'string' && parsed.narrative.trim()) {
        ...
        if (isGarbageNarrative(narrative))
            return '';
    ...
    if (Array.isArray(parsed.key_insights)) {
        const first = parsed.key_insights[0];
        if (first?.text)
            return String(first.text).slice(0, 600);
    }
    const cleaned = text.replace(/```json\s*|```\s*/g, '').trim();
...
function narrativeUsesOnlyDashboardPeriods(narrative, periods) {
    if (!periods.length)
        return true;
...
function metricsInsightFallback(entityType, metrics) {
    return metrics.tab_insights?.[entityType] || buildTabInsights(metrics)[entityType] || '';
}
export async function getSupervisorSummary(entityType, filters, metrics) {
    ...
    if (metrics?.periods?.length &&
        ...
        if (fallback) {
            console.warn(`[supervisor] ${entityType} cited periods outside dashboard (${metrics.periods.join(', ')}) — using metrics insight`);
            narrative = fallback;
        }
    }
    if (!narrative) {
...
export async function getAllSupervisorSummaries(filters, metrics) {
    ...
    const entries = await Promise.all(tabs.map(async (tab) => {
        try {
            ...
            return [tab, narrative];
        }
        catch (e) {
            console.warn(`[supervisor] ${tab} failed:`, e.message);
            return [tab, ''];
        }
    }));
