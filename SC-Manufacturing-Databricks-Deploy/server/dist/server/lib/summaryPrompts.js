...
function metricViewName() {
    return resolveMetricView();
}
const BASE_CONTEXT = (view) => `
...
function peakLowFromTrend(periods, trend) {
    if (!periods.length || !trend.length)
        return null;
    let peakI = -1;
    let lowI = -1;
    for (let i = 0; i < Math.min(periods.length, trend.length); i++) {
        ...
        if (!Number.isFinite(v) || v <= 0)
            continue;
        if (peakI < 0 || v > Number(trend[peakI]))
            peakI = i;
        if (lowI < 0 || v < Number(trend[lowI]))
            lowI = i;
    }
    if (peakI < 0 || lowI < 0)
        return null;
...
export function buildMetricsGroundingContext(metrics, entityType) {
    ...
    if (peakLow) {
        lines.push(`Dashboard peak period: ${peakLow.peakPeriod} at ${peakLow.peakPct}% DT`, `Dashboard lowest period: ${peakLow.lowPeriod} at ${peakLow.lowPct}% DT`);
    }
    ...
    if (entityType === 'category' && metrics.category_by_period) {
        ...
        if (topCats.length)
            lines.push(`Top categories (dashboard): ${topCats.join('; ')}`);
    }
    lines.push('RULE: Never mention a period label (e.g. P11) unless it appears in "Visible periods ONLY" above.');
    return lines.join('\n');
}
...
export function buildSupervisorPrompt(entityType, filters, metrics) {
